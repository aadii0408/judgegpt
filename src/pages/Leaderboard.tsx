...
import { TRACKS, type Track, type Project, type FinalReport } from "@/lib/judges";
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto max-w-3xl px-4 py-10">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-3xl font-bold flex items-center gap-2"><Trophy className="h-7 w-7 text-warning" /> Leaderboard</h2>
              ...
              <TableBody>
                {filtered.map((entry, i) => (
                  <TableRow key={entry.project.id} className="cursor-pointer hover:bg-muted/50" onClick={() => navigate(`/report/${entry.project.id}`)}>
                    ...
                    <TableCell><Badge variant="secondary">{entry.project.track}</Badge></TableCell>
                    <TableCell className={`text-right font-bold ${entry.score >= 8 ? "text-success" : entry.score >= 6 ? "text-warning" : "text-destructive"}`}>
